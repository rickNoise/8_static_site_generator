...
def split_nodes_link(old_nodes):
    ...
    for node in old_nodes:
        if node.text_type != TextType.TEXT:
            ...
            # if the text node has no links in it, just append it as is
            if len(matches) == 0:
                return_lst.append(node)
            # if it does have links in it, process differently
            else:
                processed_nodes = recursive_match_processing_for_links(matches, node.text, [])
                print(f"processed_nodes: {processed_nodes}")
                for item in processed_nodes:
                    return_lst.append(item)
...
# helper fn for split_nodes_link
def recursive_match_processing_for_links(matches, text, output_list=[]):
    print(f"entering into recursive_match_processing... with matches: {matches} and remaining_text: {text}")
    # returns an array of TextNodes
    ...
    # create a Text type TextNode for the initial text before the link
    if split_text[0] != "":
        output_list.append(
            TextNode(split_text[0], TextType.TEXT)
        )
    
    # create an Link type TextNode for the link itself
    output_list.append(
    ...
    if len(matches) > 1:
        ...
        if len(split_text) > 1:
            ...
            remaining_text = ""
        print(f"about to recurse with current output_list: {output_list}")
        output_list.extend(recursive_match_processing_for_links(remaining_matches, remaining_text, []))
        return output_list
    # if no remaining matches after this one
    else:
        # covers the trailing text left after the last image match
